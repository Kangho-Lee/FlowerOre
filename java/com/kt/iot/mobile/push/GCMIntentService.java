    ...
    private void sendIntent(Context context, Intent intent) {
        ...
        if(type.equals(PushApi.PUSH_MSG_TYPE_COLLECT)){
            ...
            try {
                ...
                if(!jsonObject1.isNull("illuminance")) {
                    int logLight = jsonObject1.getInt("illuminance");
                    light = Integer.toString(logLight);
                }

                if(!jsonObject1.isNull("SoilHumidity")) {


                    int logHum = jsonObject1.getInt("SoilHumidity");
                    Humidity = Integer.toString(logHum);
                }

                if(!jsonObject1.isNull("temperature")) {
                    int logTemp = jsonObject1.getInt("temperature");
                    temperature = Integer.toString(logTemp);
                }

             /*   Log.w(TAG, "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"+temp);
                Log.w(TAG, "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"+Integer.toString(logLight));*/
            } catch (JSONException e) {
                e.printStackTrace();
            }
            ...
            if(System.currentTimeMillis()-timeTip>5000)
            {
                PlantTip plantTip = new PlantTip();
                String plant_tip = plantTip.plantTip();
                ...
                builder.setSmallIcon(R.drawable.user).setTicker("HETT").setWhen(System.currentTimeMillis())
                        .setContentTitle("FlowerOre").setContentText(plant_tip)
